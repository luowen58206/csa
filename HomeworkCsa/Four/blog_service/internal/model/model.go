...
/*
	·通过调用scope.FieldByName 方法，获取当前是否包含所需的字段
	·通过判断Field.IsBlank 的值、可以得知该字段的值是否为空
	·若为空、则会调用 Field.Set 方法给该字段设置值、入参类型为 interface{} 内部也就是通过反射进行一系列的操作赋值
 */
...
// 删除行为的回调
/*
	·通过调用 scope.Get("gorm:delete_option")
	去获取当前设置了标识 gorm:delete_option 的字段属性。
	...
	并调用一系列方法针对 SQL 语 句的组成部分进行处理和转移，
